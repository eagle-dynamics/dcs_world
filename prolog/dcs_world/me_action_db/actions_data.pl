...
actions_data(task:payload(task{id:"NoTask",params:[]}),no_task).
...
actions_data(task:payload(task{id:"AttackGroup",params:params{attackQty:1,attackQtyLimit:false,groupAttack:false}}),attack_group).
...
actions_data(task:payload(task{id:"AttackUnit",params:params{attackQty:1,attackQtyLimit:false,groupAttack:false}}),attack_unit).
actions_data(desc("Attack the enemy unit or the enemy static object"),attack_unit).
...
actions_data(task:payload(task{id:"AttackMapObject",params:params{attackQty:1,attackQtyLimit:true,groupAttack:false}}),attack_map_object).
...
actions_data(task:payload(task{id:"Bombing",params:params{attackQty:1,attackQtyLimit:false,groupAttack:false}}),bombing).
...
actions_data(task:payload(task{id:"BombingRunway",params:params{attackQty:1,attackQtyLimit:false,groupAttack:true}}),bombing_runway).
...
actions_data(task:payload(task{id:"Orbit",params:[]}),orbit).
...
actions_data(task:payload(task{id:"Land",params:params{duration:300,durationFlag:false}}),land).
...
actions_data(task:payload(task{id:"Refueling",params:[]}),refueling).
...
actions_data(task:payload(task{id:"FAC_AttackGroup",params:[]}),fac_assign_group).
actions_data(desc("Make the lead unit of the group FAC and assign it the target to designate"),fac_assign_group).
...
actions_data(task:payload(task{id:"FireAtPoint",params:params{expendQty:1,expendQtyEnabled:false,templateId:"",zoneRadius:0}}),fire_at_point).
...
actions_data(task:payload(task{id:"Hold",params:params{templateId:""}}),hold).
...
actions_data(task:payload(task{id:"Follow",params:params{lastWptIndexFlag:true,lastWptIndexFlagChangedManually:true,pos:pos{x: -500,y:0,z:200}}}),follow).
...
actions_data(task(params(last_wpt_index_flag_changed_manually(true))),follow).
...
actions_data(task:payload(task{id:"Escort",params:params{engagementDistMax:60000,lastWptIndexFlag:true,lastWptIndexFlagChangedManually:true,pos:pos{x: -500,y:0,z:200},targetTypes:["Planes"]}}),escort).
actions_data(desc("Escort the another group: follow it and protect it from specific types of threats"),escort).
...
actions_data(task(params(last_wpt_index_flag_changed_manually(true))),escort).
...
actions_data(task:payload(task{id:"EmbarkToTransport",params:params{zoneRadius:200}}),embark_to_transport).
...
actions_data(task:payload(task{id:"GoToWaypoint",params:[]}),go_to_waypoint).
...
actions_data(task:payload(task{id:"Embarking",params:params{distribution:[],distributionFlag:false,duration:300,durationFlag:false,groupsForEmbarking:[]}}),embarking).
...
actions_data(task:payload(task{id:"Disembarking",params:params{groupsForEmbarking:[]}}),disembarking).
...
actions_data(task:payload(task{id:"CargoTransportation",params:[]}),cargo_transportation).
...
actions_data(task:payload(task{id:"GroundEscort",params:params{engagementDistMax:500,lastWptIndexFlag:true,lastWptIndexFlagChangedManually:true,targetTypes:["Helicopters","Ground Units"]}}),ground_escort).
actions_data(desc("Escort the ground vehicle group: follow it and protect it from specific types of threats"),ground_escort).
...
actions_data(task(params(last_wpt_index_flag_changed_manually(true))),ground_escort).
...
actions_data(task:payload(task{id:"NoTask",params:[]}),no_enroute_task).
...
actions_data(task:payload(task{id:"EngageTargets",params:params{maxDist:15000,maxDistEnabled:false}}),search_then_engage).
actions_data(desc("Engage targets of specific types along the route"),search_then_engage).
...
actions_data(task:payload(task{id:"EngageTargetsInZone",params:params{zoneRadius:5000}}),search_then_engage_in_zone).
actions_data(desc("Engage targets of specific types in the given zone"),search_then_engage_in_zone).
actions_data(display_name("Search Then Engage In Zone"),search_then_engage_in_zone).
actions_data(task(id("EngageTargetsInZone")),search_then_engage_in_zone).
actions_data(task(params(zone_radius(5000))),search_then_engage_in_zone).
actions_data(type(2),search_then_engage_in_zone).
actions_data(id(24),search_then_engage_group).
actions_data(task:payload(task{id:"EngageGroup",params:params{priority:1}}),search_then_engage_group).
actions_data(desc("Allow the group to engage the enemy group during the mission"),search_then_engage_group).
actions_data(display_name("Search Then Engage Group"),search_then_engage_group).
...
actions_data(task:payload(task{id:"EngageUnit",params:params{attackQty:1,attackQtyLimit:false,groupAttack:false,priority:1}}),search_then_engage_unit).
actions_data(desc("Allow the group to engage the enemy unit or the enemy static object during the mission"),search_then_engage_unit).
actions_data(display_name("Search Then Engage Unit"),search_then_engage_unit).
actions_data(task(id("EngageUnit")),search_then_engage_unit).
actions_data(task(params(attack_qty(1))),search_then_engage_unit).
actions_data(task(params(attack_qty_limit(false))),search_then_engage_unit).
...
actions_data(task:payload(task{id:"AWACS",params:[]}),awacs).
...
actions_data(task:payload(task{id:"EWR",params:[]}),ewr).
...
actions_data(task:payload(task{id:"FAC",params:[]}),fac).
actions_data(desc("Make the lead aircraft of the a FAC and let it to choose targets to assign by its own"),fac).
actions_data(display_name("FAC"),fac).
actions_data(task(id("FAC")),fac).
actions_data(type(2),fac).
actions_data(id(29),fac_engage_group).
actions_data(task:payload(task{id:"FAC_EngageGroup",params:[]}),fac_engage_group).
actions_data(desc("Make the lead aircraft of the group FAC and allow it to assign the enemy group"),fac_engage_group).
...
actions_data(task:payload(task{id:"Tanker",params:[]}),tanker).
...
actions_data(task:payload(task{id:"EngageTargets",key:"FighterSweep",params:params{priority:0,targetTypes:["Planes"]}}),fighter_sweep).
actions_data(desc("Engage enemy aircraft. Enemy fighters are prioritiest targets"),fighter_sweep).
...
actions_data(task:payload(task{id:"EngageTargets",key:"CAS",params:params{priority:0,targetTypes:["Helicopters","Ground Units","Light armed ships"]}}),cas).
...
actions_data(task:payload(task{id:"EngageTargets",key:"CAP",params:params{priority:0,targetTypes:["Air"]}}),cap).
...
actions_data(task:payload(task{id:"EngageTargets",key:"SEAD",params:params{priority:0,targetTypes:["Air Defence"]}}),sead).
...
actions_data(task:payload(task{id:"EngageTargets",key:"AntiShip",params:params{priority:0,targetTypes:["Ships"]}}),anti_ship).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"NoAction",params:[]}}}),no_action).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Script",params:params{command:""}}}}),run_script).
actions_data(desc("Run lua script. The group will be passed as a single parameter to the function."),run_script).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"ScriptFile",params:params{file:""}}}}),script_file).
actions_data(desc("Run lua script file. The group will be passed as a single parameter to the function."),script_file).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SetCallsign",params:[]}}}),set_callsign).
actions_data(desc("Set callname and group number to the group."),set_callsign).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SetFrequency",params:params{frequency:131000000,modulation:0,power:10}}}}),set_frequency).
actions_data(desc("Set frequency to radios of all the units in the group."),set_frequency).
...
actions_data(task(params(action(params(frequency(131000000))))),set_frequency).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SetFrequencyForUnit",params:params{frequency:131000000,modulation:0,power:10}}}}),set_frequency_for_unit).
actions_data(desc("Set frequency to radios to the unit"),set_frequency_for_unit).
actions_data(display_name("Set Frequency for unit"),set_frequency_for_unit).
actions_data(task(id("WrappedAction")),set_frequency_for_unit).
actions_data(task(params(action(id("SetFrequencyForUnit")))),set_frequency_for_unit).
actions_data(task(params(action(params(frequency(131000000))))),set_frequency_for_unit).
actions_data(task(params(action(params(modulation(0))))),set_frequency_for_unit).
actions_data(task(params(action(params(power(10))))),set_frequency_for_unit).
actions_data(type(3),set_frequency_for_unit).
actions_data(id(42),transmit_message).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"TransmitMessage",params:params{duration:5,file:"",loop:false,subtitle:""}}}}),transmit_message).
actions_data(desc("Start radio transmission from the lead unit of the group"),transmit_message).
actions_data(display_name("Transmit Message"),transmit_message).
actions_data(task(id("WrappedAction")),transmit_message).
actions_data(task(params(action(id("TransmitMessage")))),transmit_message).
actions_data(task(params(action(params(duration(5))))),transmit_message).
actions_data(task(params(action(params(file(""))))),transmit_message).
actions_data(task(params(action(params(loop(false))))),transmit_message).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"StopTransmission",params:[]}}}),stop_transmission).
actions_data(desc("Stop the radio transmission from the lead unit of the group"),stop_transmission).
actions_data(display_name("Stop Transmission"),stop_transmission).
actions_data(task(id("WrappedAction")),stop_transmission).
actions_data(task(params(action(id("StopTransmission")))),stop_transmission).
actions_data(type(3),stop_transmission).
actions_data(id(44),switch_waypoint).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SwitchWaypoint",params:[]}}}),switch_waypoint).
actions_data(desc("Switch current waypoint of the route."),switch_waypoint).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SwitchAction",params:params{actionIndex:1}}}}),switch_action).
actions_data(desc("Jump to another action in the action list of the waypoint."),switch_action).
...
actions_data(task(params(action(params(action_index(1))))),switch_action).
actions_data(type(3),switch_action).
actions_data(id(46),invisible).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SetInvisible",params:params{value:true}}}}),invisible).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SetImmortal",params:params{value:true}}}}),immortal).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Start",params:[]}}}),start).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"ActivateBeacon",params:params{bearing:true,callsign:"TKR",channel:1,frequency:1088000000,modeChannel:"X",system:4,type:4}}}}),activate_tacan).
actions_data(desc("Activate TACAN beacon onboard of the group lead unit. Only one beacon is available."),activate_tacan).
...
actions_data(task(params(action(params(bearing(true))))),activate_tacan).
actions_data(task(params(action(params(callsign("TKR"))))),activate_tacan).
actions_data(task(params(action(params(channel(1))))),activate_tacan).
actions_data(task(params(action(params(frequency(1088000000))))),activate_tacan).
actions_data(task(params(action(params(mode_channel("X"))))),activate_tacan).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"DeactivateBeacon",params:[]}}}),deactivate_beacon).
actions_data(desc("Dectivate active beacon (TACAN, etc) onboard of the group lead unit."),deactivate_beacon).
actions_data(display_name("Deactivate beacon"),deactivate_beacon).
actions_data(task(id("WrappedAction")),deactivate_beacon).
actions_data(task(params(action(id("DeactivateBeacon")))),deactivate_beacon).
actions_data(type(3),deactivate_beacon).
actions_data(id(51),eplrs).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"EPLRS",params:params{groupId:0,value:true}}}}),eplrs).
actions_data(desc("Swich Enhanced Position Location Reporting System on and off."),eplrs).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"SMOKE_ON_OFF",params:params{value:true}}}}),smoke_on_off).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name: -1}}}}),no_option).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:0}}}}),roe).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:1}}}}),reaction_to_threat).
actions_data(desc("Set what behavior is allowed as a reaction on a threat."),reaction_to_threat).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:3}}}}),radar_using).
actions_data(desc("Set the conditions when radar using is allowed to the group."),radar_using).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:4}}}}),chaff_flare_using).
actions_data(desc("Set the conditions when flare using is allowed to the group."),chaff_flare_using).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:5}}}}),formation).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:6,value:true}}}}),rtb_on_bingo_fuel).
actions_data(desc("Allows the group to return to base when it is bingo fuel."),rtb_on_bingo_fuel).
...
actions_data(task(params(action(params(value(true))))),rtb_on_bingo_fuel).
actions_data(type(4),rtb_on_bingo_fuel).
actions_data(id(60),rtb_on_out_of_ammo).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:10}}}}),rtb_on_out_of_ammo).
actions_data(desc("Set specific type of ammo that is required by the group to continue its mission."),rtb_on_out_of_ammo).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:7,value:false}}}}),silence).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:8,value:600}}}}),disperse_under_fire).
actions_data(desc("Allow the group to disperse under attack from the air and set the delay."),disperse_under_fire).
...
actions_data(task(params(action(params(value(600))))),disperse_under_fire).
actions_data(type(4),disperse_under_fire).
actions_data(id(63),alarm_state).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:9}}}}),alarm_state).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:13}}}}),ecm_using).
actions_data(desc("Set the conditions when ECM using is allowed to the group."),ecm_using).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:14,value:false}}}}),restrict_air_to_air_attack).
actions_data(desc("Restrict Air-to-Air Attack"),restrict_air_to_air_attack).
actions_data(display_name("Restrict Air-to-Air Attack"),restrict_air_to_air_attack).
actions_data(task(id("WrappedAction")),restrict_air_to_air_attack).
actions_data(task(params(action(id("Option")))),restrict_air_to_air_attack).
actions_data(task(params(action(params(name(14))))),restrict_air_to_air_attack).
actions_data(task(params(action(params(value(false))))),restrict_air_to_air_attack).
actions_data(type(4),restrict_air_to_air_attack).
actions_data(id(66),restrict_jettison).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:15,value:false}}}}),restrict_jettison).
actions_data(desc("Restrict Jettisoning external stores."),restrict_jettison).
...
actions_data(task(params(action(params(value(false))))),restrict_jettison).
actions_data(type(4),restrict_jettison).
actions_data(id(67),restrict_afterburner).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:16,value:false}}}}),restrict_afterburner).
actions_data(desc("Restrict Afterburner usage en route."),restrict_afterburner).
...
actions_data(task(params(action(params(name(16))))),restrict_afterburner).
actions_data(task(params(action(params(value(false))))),restrict_afterburner).
actions_data(type(4),restrict_afterburner).
actions_data(id(68),restrict_air_to_ground_attack).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:17,value:false}}}}),restrict_air_to_ground_attack).
actions_data(desc("Restrict Air-to-Ground Attack."),restrict_air_to_ground_attack).
actions_data(display_name("Restrict Air-to-Ground Attack"),restrict_air_to_ground_attack).
actions_data(task(id("WrappedAction")),restrict_air_to_ground_attack).
actions_data(task(params(action(id("Option")))),restrict_air_to_ground_attack).
actions_data(task(params(action(params(name(17))))),restrict_air_to_ground_attack).
actions_data(task(params(action(params(value(false))))),restrict_air_to_ground_attack).
actions_data(type(4),restrict_air_to_ground_attack).
actions_data(id(69),aa_missile_attack_ranges).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:18}}}}),aa_missile_attack_ranges).
actions_data(desc("Choose the rule of first missile launch in BVR"),aa_missile_attack_ranges).
actions_data(display_name("AA Missile attack ranges"),aa_missile_attack_ranges).
...
actions_data(task(params(action(params(name(18))))),aa_missile_attack_ranges).
actions_data(type(4),aa_missile_attack_ranges).
actions_data(id(70),no_report_waypoint_pass).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:19,value:false}}}}),no_report_waypoint_pass).
actions_data(desc("Deny AI flights report when passing waypoint."),no_report_waypoint_pass).
actions_data(display_name("No Report Waypoint Pass"),no_report_waypoint_pass).
...
actions_data(task(params(action(params(name(19))))),no_report_waypoint_pass).
actions_data(task(params(action(params(value(false))))),no_report_waypoint_pass).
actions_data(type(4),no_report_waypoint_pass).
actions_data(id(71),aerobatics).
actions_data(task:payload(task{id:"Aerobatics",params:params{maneuversParams:[]}}),aerobatics).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:20,value:true}}}}),engage_air_weapons).
...
actions_data(task(params(action(params(value(true))))),engage_air_weapons).
actions_data(type(4),engage_air_weapons).
actions_data(id(73),ww2_carpet_bombing).
actions_data(task:payload(task{id:"CarpetBombing",params:params{attackQty:1,attackQtyLimit:false,attackType:"Carpet",carpetLength:500,groupAttack:false,weaponType:weaponType{name:"--Iron bombs",value:240}}}),ww2_carpet_bombing).
...
actions_data(task(params(weapon_type(name("--Iron bombs")))),ww2_carpet_bombing).
...
actions_data(task:payload(task{id:"FollowBigFormation",params:params{formationType:0,lastWptIndexFlag:true,lastWptIndexFlagChangedManually:true,pos:pos{x: -150,y:0,z:150},posInBox:1,posInGroup:1,posInWing:1}}),ww2_big_formation).
...
actions_data(task(params(last_wpt_index_flag_changed_manually(true))),ww2_big_formation).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:21}}}}),radio_usage_when_contact).
actions_data(desc("Set specific type of events and target types for radio messages when contact enemy"),radio_usage_when_contact).
actions_data(display_name("Radio usage when contact"),radio_usage_when_contact).
...
actions_data(task(params(action(params(name(21))))),radio_usage_when_contact).
actions_data(type(4),radio_usage_when_contact).
actions_data(id(76),radio_usage_when_engage).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:22}}}}),radio_usage_when_engage).
actions_data(desc("Set specific type of events and target types for radio messages when engage enemy"),radio_usage_when_engage).
actions_data(display_name("Radio usage when engage"),radio_usage_when_engage).
...
actions_data(task(params(action(params(name(22))))),radio_usage_when_engage).
actions_data(type(4),radio_usage_when_engage).
actions_data(id(77),radio_usage_when_kill_target).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"Option",params:params{name:23}}}}),radio_usage_when_kill_target).
actions_data(desc("Set specific type of events and target types for radio messages when kill enemy"),radio_usage_when_kill_target).
actions_data(display_name("Radio usage when kill target"),radio_usage_when_kill_target).
actions_data(task(id("WrappedAction")),radio_usage_when_kill_target).
actions_data(task(params(action(id("Option")))),radio_usage_when_kill_target).
actions_data(task(params(action(params(name(23))))),radio_usage_when_kill_target).
actions_data(type(4),radio_usage_when_kill_target).
actions_data(id(78),activate_icls).
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"ActivateICLS",params:params{channel:1,type:131584}}}}),activate_icls).
actions_data(desc("Activate ICLS onboard of the group lead unit. Only one ICLS is available."),activate_icls).
...
actions_data(task:payload(task{id:"WrappedAction",params:params{action:action{id:"DeactivateICLS",params:[]}}}),deactivate_icls).
actions_data(desc("Dectivate active beacon (ICLS, etc) onboard of the group lead unit."),deactivate_icls).
...
actions_data(task:payload(task{id:"ParatroopersDrop",params:params{attackQty:1,attackQtyLimit:false,attackType:"Carpet",carpetLength:500,groupAttack:false,groupsForAirdrop:[],scriptFileName:"",selectedGroup: -1}}),drop_of_paratroopers).
actions_data(desc("Perform paratroopers drop at target"),drop_of_paratroopers).
...
actions_data(task(params(attack_qty_limit(false))),drop_of_paratroopers).
...
actions_data(task:payload(task{id:"AirboneDropOperation",params:params{dropType:"Troops",groupDrop:false,runsQty:1}}),airbone_drop_operation).
actions_data(desc("Airbone drop operation"),airbone_drop_operation).
actions_data(display_name("Airbone drop operation"),airbone_drop_operation).
